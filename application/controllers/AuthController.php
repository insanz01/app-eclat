...
class AuthController extends CI_Controller
{
  ...
  {
    if($this->session->has_userdata('SESS_KANIGARA_USERID')) {
			redirect('dashboard');
		}
  ...
  {
		if($this->session->userdata("SESS_KANIGARA_ROLEID") == 2) {
			$this->session->unset_userdata("SESS_KANIGARA_NIP");
		}
		
		$this->session->unset_userdata("SESS_KANIGARA_USERID");
		$this->session->unset_userdata("SESS_KANIGARA_USERNAME");
		$this->session->unset_userdata("SESS_KANIGARA_ROLEID");
